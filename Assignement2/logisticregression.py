...
def thetaTransposeX(theta, X):
  if (X.ndim == 1):
    return int(theta[0] + theta[1] * X[1])
  else:
    ans = theta[0]
...
def predict(X,theta):
  ...
  if (X.ndim == 1):
    pred = np.sign(thetaTransposeX(theta, X))
  else:
    pred = []
    for i in X:
      pred.append(np.sign(thetaTransposeX(theta, i)))
  return pred
...
def computeCost(X, y, theta):
  ...
  for i in range(0, len(y)):
    th = (thetaTransposeX(theta, X[i]))
    exponent = (-1 * y) * thetaTransposeX(theta, X[i])
    lgs.append(np.log(2, (1 + np.exp(exponent))))
  cost = ((1 / len(y)) * sum(lgs))
  return cost[0]
...
def computeGradient(X,y,theta):
  ...
  for j in range(n):
    ...
    for i in range(len(y)):
      e = np.exp((-1 * y[i]) * (thetaTransposeX(theta,X[i])))
      s.append((-1 * y[i]) * X[i][j] * (e / (1 + e)))
    grad[j] = ((1 / len(y)) * sum(s))
